    ...
    [](EEOperation operation)
    {
      switch (operation)
      {
        ...
        case EEOperation::ParallelSubtractUnsignedSaturateWord:
          return ExpectedExecutionClassification{
            EEExecutionFamily::PackedArithmetic,
            EEExecutionDispatch::Immediate
          };
...
TEST_CASE("EE full-width parallel HI LO transfer decoder and dependencies")
{
  struct Contract
  {
    ...
    EEOperation operation;
    std::uint32_t gprReads;
    std::uint32_t gprWrites;
    std::uint16_t specialReads;
    std::uint16_t specialWrites;
  };
  const Contract contracts[] = {
    {
      0x09,
  ...
  for (const Contract &contract : contracts)
  {
    ...
    REQUIRE(decoded.operation == contract.operation);
    REQUIRE(dependencies.gprReads == contract.gprReads);
    REQUIRE(dependencies.gprWrites == contract.gprWrites);
    REQUIRE(dependencies.specialReads == contract.specialReads);
    REQUIRE(dependencies.specialWrites == contract.specialWrites);
    const EEInstructionRouting routing =
      eeInstructionRouting(decoded.operation);
  ...
  REQUIRE_THROWS_AS(
    decodeEEInstruction(
      UINT32_C(0x70000000) |
      registerInstruction(0x09, 1, 0, 3, 0x08)),
    EEInstructionDecodeError);
  REQUIRE_THROWS_AS(
    decodeEEInstruction(
      UINT32_C(0x70000000) |
      registerInstruction(0x09, 0, 1, 3, 0x09)),
    EEInstructionDecodeError);
  REQUIRE_THROWS_AS(
    decodeEEInstruction(
      UINT32_C(0x70000000) |
      registerInstruction(0x29, 1, 1, 0, 0x08)),
    EEInstructionDecodeError);
  REQUIRE_THROWS_AS(
    decodeEEInstruction(
      UINT32_C(0x70000000) |
...
TEST_CASE("EE partial parallel HI LO write decoder and dependencies")
{
  const EEInstruction decoded =
    decodeEEInstruction(
      UINT32_C(0x70000000) |
  ...
  REQUIRE_THROWS_AS(
    decodeEEInstruction(
      UINT32_C(0x70000000) |
      registerInstruction(0x31, 1, 1, 0, 0)),
    EEInstructionDecodeError);
  REQUIRE_THROWS_AS(
    decodeEEInstruction(
      UINT32_C(0x70000000) |
      registerInstruction(0x31, 1, 0, 1, 0)),
    EEInstructionDecodeError);
  REQUIRE_THROWS_AS(
    decodeEEInstruction(
      UINT32_C(0x70000000) |
      registerInstruction(0x31, 1, 0, 0, 1)),
    EEInstructionDecodeError);
}
...
TEST_CASE("EE wrapping packed arithmetic decoder and dependencies")
{
  struct Contract
  {
    std::uint8_t nestedFunction;
  ...
  const Contract contracts[] = {
    {0x08, EEOperation::ParallelAddByte},
    {0x04, EEOperation::ParallelAddHalfword},
    {0x00, EEOperation::ParallelAddWord},
    {0x09, EEOperation::ParallelSubtractByte},
    {0x05, EEOperation::ParallelSubtractHalfword},
    {0x01, EEOperation::ParallelSubtractWord}
  };
...
TEST_CASE("EE mixed packed halfword arithmetic decoder and dependencies")
{
  const EEInstruction decoded =
    decodeEEInstruction(
      UINT32_C(0x70000000) |
      registerInstruction(0x28, 1, 2, 3, 0x04));
  const EEInstructionDependencies dependencies =
    eeInstructionDependencies(decoded);
...
TEST_CASE("EE signed saturating packed arithmetic decoder and dependencies")
{
  struct Contract
  {
    std::uint8_t nestedFunction;
  ...
  const Contract contracts[] = {
    {0x18, EEOperation::ParallelAddSignedSaturateByte},
    {0x14, EEOperation::ParallelAddSignedSaturateHalfword},
    {0x10, EEOperation::ParallelAddSignedSaturateWord},
    {0x19, EEOperation::ParallelSubtractSignedSaturateByte},
    {0x15, EEOperation::ParallelSubtractSignedSaturateHalfword},
    {0x11, EEOperation::ParallelSubtractSignedSaturateWord}
  };
...
TEST_CASE("EE unsigned saturating packed arithmetic decoder and dependencies")
{
  struct Contract
  {
    std::uint8_t nestedFunction;
  ...
  const Contract contracts[] = {
    {0x18, EEOperation::ParallelAddUnsignedSaturateByte},
    {0x14, EEOperation::ParallelAddUnsignedSaturateHalfword},
    {0x10, EEOperation::ParallelAddUnsignedSaturateWord},
    {0x19, EEOperation::ParallelSubtractUnsignedSaturateByte},
    {0x15, EEOperation::ParallelSubtractUnsignedSaturateHalfword},
    {0x11, EEOperation::ParallelSubtractUnsignedSaturateWord}
  };
...
TEST_CASE("EE lower packed interleave decoder and dependencies")
{
  struct Contract
  {
    std::uint8_t nestedFunction;
  ...
  const Contract contracts[] = {
    {0x1a, EEOperation::ParallelExtendLowerByte},
    {0x16, EEOperation::ParallelExtendLowerHalfword},
    {0x12, EEOperation::ParallelExtendLowerWord}
  };
...
TEST_CASE("EE upper packed interleave decoder and dependencies")
{
  struct Contract
  {
    std::uint8_t nestedFunction;
  ...
  const Contract contracts[] = {
    {0x1a, EEOperation::ParallelExtendUpperByte},
    {0x16, EEOperation::ParallelExtendUpperHalfword},
    {0x12, EEOperation::ParallelExtendUpperWord}
  };
...
TEST_CASE("EE truncating packed pack decoder and dependencies")
{
  struct Contract
  {
    std::uint8_t nestedFunction;
  ...
  const Contract contracts[] = {
    {0x1b, EEOperation::ParallelPackToByte},
    {0x17, EEOperation::ParallelPackToHalfword},
    {0x13, EEOperation::ParallelPackToWord}
  };
...
TEST_CASE("EE selected packed halfword interleave decoder and dependencies")
{
  struct Contract
  {
    std::uint8_t function;
  ...
  const Contract contracts[] = {
    {0x09, EEOperation::ParallelInterleaveHalfword},
    {0x29, EEOperation::ParallelInterleaveEvenHalfword}
  };
...
TEST_CASE("EE packed copy decoder and dependencies")
{
  struct Contract
  {
    std::uint8_t function;
  ...
  for (const Contract &contract : contracts)
  {
    const std::uint8_t source =
      contract.operation == EEOperation::ParallelCopyHalfword
        ? 0
        : 1;
    const EEInstruction decoded =
      decodeEEInstruction(
    ...
    REQUIRE(decoded.operation == contract.operation);
    REQUIRE(dependencies.gprReads == contract.reads);
    REQUIRE(dependencies.gprWrites == (UINT32_C(1) << 3));
    REQUIRE(dependencies.specialReads == 0);
    REQUIRE(dependencies.specialWrites == 0);
    const EEInstructionRouting routing =
  ...
  REQUIRE_THROWS_AS(
    decodeEEInstruction(
      UINT32_C(0x70000000) |
      registerInstruction(0x29, 1, 2, 3, 0x1b)),
    EEInstructionDecodeError);
}
...
TEST_CASE("EE packed reverse and rotate decoder and dependencies")
{
  struct Contract
  {
    std::uint8_t nestedFunction;
  ...
  const Contract contracts[] = {
    {0x1b, EEOperation::ParallelReverseHalfword},
    {0x1f, EEOperation::ParallelRotateThreeWords}
  };
...
TEST_CASE("EE packed pixel format decoder and dependencies")
{
  struct Contract
  {
    std::uint8_t nestedFunction;
  ...
  const Contract contracts[] = {
    {0x1e, EEOperation::ParallelExtendFiveBit},
    {0x1f, EEOperation::ParallelPackFiveBit}
  };
...
TEST_CASE("EE immediate packed word shift decoder and dependencies")
{
  struct Contract
  {
    std::uint8_t function;
  ...
  const Contract contracts[] = {
    {0x3c, EEOperation::ParallelShiftLeftLogicalWord},
    {0x3e, EEOperation::ParallelShiftRightLogicalWord},
    {0x3f, EEOperation::ParallelShiftRightArithmeticWord}
  };
...
TEST_CASE("EE variable packed word shift decoder and dependencies")
{
  struct Contract
  {
    std::uint8_t primaryFunction;
