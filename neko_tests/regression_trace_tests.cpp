...
TEST_CASE("EE COP1 move traces the LS and C1 stage boundaries")
{
  NekoSystem system;
  EECore &core = system.eeCore();
  ...
  REQUIRE(
    pipelineEvents[4].type ==
    NekoTraceEventType::COP1Retired);
  REQUIRE(pipelineEvents[4].masterCycle == 4);
  REQUIRE(pipelineEvents[4].value0 == 1);
  REQUIRE(
    pipelineEvents[4].value2 ==
    (UINT64_C(0x89abcdef) |
     (NekoEETraceCOP1Result::DESTINATION_GPR <<
      NekoEETraceCOP1Result::DESTINATION_MASK_SHIFT) |
     (UINT64_C(2) <<
...
TEST_CASE("EE packed copy traces and hashes are deterministic")
{
  const auto prepare =
    [](NekoSystem *system)
    {
