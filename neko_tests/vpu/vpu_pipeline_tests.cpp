...
TEST_CASE("VPU Pipeline Tests")
{
  ...
  {
    orchestrator.initPipeline(VPU_PIPELINE_TYPE_FMAC, 0, 0, 0, 0, 0, VPU_REGISTER_VF02, 0, VPU_REGISTER_VF01, VPU_BC_NONE, FP_REGISTER_X_FIELD);
  ...
  SECTION("When a pipeline finishes, we can retrieve the correct information")
  {
    ...
    orchestrator.initPipeline(VPU_PIPELINE_TYPE_FMAC, 10, 20, 30, 40, 50, VPU_REGISTER_VF02, VPU_REGISTER_VF03, VPU_REGISTER_VF01, VPU_BC_Z, FP_REGISTER_X_FIELD);
    ...
    REQUIRE(pipeline->bcField == VPU_BC_Z);
  }
  ...
  SECTION("Two FMAC Pipelines execute in 5 cycles with no stall")
  {
    ...
    orchestrator.update();
    cycles++;
    orchestrator.initPipeline(VPU_PIPELINE_TYPE_FMAC, 0, 0, 0, 0, 0, VPU_REGISTER_VF03, 0, VPU_REGISTER_VF01, VPU_BC_NONE, FP_REGISTER_X_FIELD);
  ...
  {
    orchestrator.initPipeline(VPU_PIPELINE_TYPE_FMAC, 0, 0, 0, 0, 0, VPU_REGISTER_VF01, 0, VPU_REGISTER_VF02, VPU_BC_NONE, FP_REGISTER_X_FIELD | FP_REGISTER_Y_FIELD);
    orchestrator.update();
    cycles++;
    orchestrator.initPipeline(VPU_PIPELINE_TYPE_FMAC, 0, 0, 0, 0, 0, VPU_REGISTER_VF02, 0, VPU_REGISTER_VF03, VPU_BC_NONE, FP_REGISTER_Y_FIELD);
  ...
  SECTION("The PipelineOrchestrator throws an exception if we try to add more pipelines than the maximum allowed")
  {
    ...
    {
      orchestrator.initPipeline(VPU_PIPELINE_TYPE_FMAC, 0, 0, 0, 0, 0, VPU_REGISTER_VF02, VPU_REGISTER_VF03, VPU_REGISTER_VF01, VPU_BC_W, 0);
    }

    REQUIRE_THROWS_WITH(orchestrator.initPipeline(VPU_PIPELINE_TYPE_FMAC, 0, 0, 0, 0, 0, VPU_REGISTER_VF02, VPU_REGISTER_VF03, VPU_REGISTER_VF01, VPU_BC_Y, 0), Contains("Trying to add a pipeline to the PipelineOrchestrator when the max number of pipelines is already in use!"));
  }
}
