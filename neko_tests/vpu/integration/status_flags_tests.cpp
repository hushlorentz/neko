...
TEST_CASE("VU0 status flag integration program")
{
  VPU vpu;
  ...
  config.microProgram = vpu_integration::readBinary("status_flags.bin");
  ...
  vpu_integration::appendQword(&expected, 0x82, 1, 0xc7, 0xfc0);

  REQUIRE(result.state == VPU_STATE_READY);
  REQUIRE(result.elapsedCycles == 38);
