...
namespace
{
  ...
  const NestedMmiDecodeTable &mmi0Table()
  {
    static const NestedMmiDecodeTable table = []()
    {
      ...
      result[0x1e] = {
        DecodeKind::Direct,
        EEOperation::ParallelExtendFiveBit,
        REGISTER_SOURCE_MASK
      };
      result[0x1f] = {
        DecodeKind::Direct,
        EEOperation::ParallelPackFiveBit,
        REGISTER_SOURCE_MASK
      };
      return result;
    }();
...
bool isPackedMultiplyOperation(EEOperation operation)
{
  return
    ...
    operation == EEOperation::ParallelMultiplyAddWord ||
    operation == EEOperation::ParallelMultiplyAddUnsignedWord ||
    operation == EEOperation::ParallelMultiplySubtractWord ||
    operation == EEOperation::ParallelMultiplyHalfword ||
    operation == EEOperation::ParallelMultiplyAddHalfword ||
    operation == EEOperation::ParallelMultiplySubtractHalfword ||
    operation ==
      EEOperation::ParallelHorizontalMultiplyAddHalfword ||
    operation ==
      EEOperation::ParallelHorizontalMultiplySubtractHalfword;
...
EEInstruction decodeEEInstruction(std::uint32_t raw)
{
  ...
  if (primary.kind == DecodeKind::Mmi)
  {
    ...
    if (mmi.kind == DecodeKind::Pmfhl)
    {
      ...
      switch (instruction.shiftAmount)
      {
        case 0:
          instruction.operation =
